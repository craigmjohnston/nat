...
- `internal/cli/` — the headless commands (`nat info [--json]`,
  ...
  closes out a slice the configured user holds,
  `nat release-slice <slice>`, which hands one back the other way — Status to
  `Todo`, the Assignee cleared and one line on the page saying so, for a session
  that ended without finishing — and the one-off additions
