...
func TestGetView(t *testing.T) {
	t.Run("fetches one view in full", func(t *testing.T) {
		var gotMethod, gotPath string
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			gotMethod, gotPath = r.Method, r.URL.EscapedPath()
			w.Write([]byte(`{"object":"view","id":"v-1","name":"Default view","type":"table"}`))
		}))
		defer srv.Close()

		c, _ := testClient(t, srv)
		view, err := c.GetView(context.Background(), "v 1")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if gotMethod != http.MethodGet || gotPath != "/views/v%201" {
			t.Errorf("got %s %s, want GET /views/v%%201", gotMethod, gotPath)
		}
		if want := (View{ID: "v-1", Name: "Default view", Type: ViewTypeTable}); *view != want {
	...
	t.Run("propagates an API error", func(t *testing.T) {
		...
		c, _ := testClient(t, srv)
		if view, err := c.GetView(context.Background(), "v-1"); err == nil || view != nil {
			t.Fatalf("got %+v, %v; want nil and an error", view, err)
		}
	})
}

func TestCreateBoardView(t *testing.T) {
	t.Run("creates a board grouped by the select in manual order", func(t *testing.T) {
		var gotMethod, gotPath, gotBody string
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			gotMethod, gotPath = r.Method, r.URL.Path
			body, _ := io.ReadAll(r.Body)
			gotBody = string(body)
			w.Write([]byte(`{"object":"view","id":"v-9","name":"Board","type":"board"}`))
		}))
		defer srv.Close()

		c, _ := testClient(t, srv)
		view, err := c.CreateBoardView(context.Background(), "db-1", "ds-1", "Board", "prop-1")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if gotMethod != http.MethodPost || gotPath != "/views" {
			t.Errorf("got %s %s, want POST /views", gotMethod, gotPath)
		}
		want := `{"configuration":{"group_by":{"property_id":"prop-1","sort":{"type":"manual"},"type":"select"},` +
			`"type":"board"},"data_source_id":"ds-1","database_id":"db-1","name":"Board","type":"board"}`
		if gotBody != want {
			t.Errorf("body = %s, want %s", gotBody, want)
		}
		if want := (View{ID: "v-9", Name: "Board", Type: ViewTypeBoard}); *view != want {
			t.Errorf("view = %+v, want %+v", view, want)
		}
	})
	...
	t.Run("propagates an API error", func(t *testing.T) {
		...
		c, _ := testClient(t, srv)
		if view, err := c.CreateBoardView(context.Background(), "db-1", "ds-1", "Board", "prop-1"); err == nil || view != nil {
			t.Fatalf("got %+v, %v; want nil and an error", view, err)
		}
	})
}
...
func TestDeleteView(t *testing.T) {
	t.Run("deletes the view", func(t *testing.T) {
		var gotMethod, gotPath string
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			gotMethod, gotPath = r.Method, r.URL.EscapedPath()
			w.Write([]byte(`{"object":"view","id":"v-1"}`))
		}))
		defer srv.Close()

		c, _ := testClient(t, srv)
		if err := c.DeleteView(context.Background(), "v 1"); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if gotMethod != http.MethodDelete || gotPath != "/views/v%201" {
			t.Errorf("got %s %s, want DELETE /views/v%%201", gotMethod, gotPath)
		}
	})
